...
const CreateDemandeur = () => {
  ...
  useEffect(() => {
    const fetchClients = async () => {
      try {
        ...
        // If clientId is provided in URL, set it as selected
        if (clientId) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log("Submitting demandeur:", formData)
      const res = await axios.post("/api/demandeurs", formData)
      console.log("Demandeur created:", res.data)

      // Navigate back to clients page or to the specific client
      if (clientId) {
      ...
      console.error("Error creating demandeur:", err)
  ...
  return (
    <Layout>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6 transition-all duration-200 hover:shadow-md">
        <h1 className="text-2xl font-bold mb-6 text-slate-800">Ajouter un nouveau demandeur</h1>

        {selectedClient && (
          <div className="bg-teal-50 border border-teal-200 text-teal-700 px-4 py-3 rounded-lg mb-6 flex items-center">
            <svg
              xmlns="http://www.w3.org/2000/svg"
...
export default CreateDemandeur
